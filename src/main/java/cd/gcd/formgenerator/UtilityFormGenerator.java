...
public class UtilityFormGenerator {
	...
	public static <T> TableFilter<T> createTableView(Class<T> classe) {
		TableView<T> tview = new TableView<T>();
		...
		for (int var6 = 0; var6 < var5; ++var6) {
			...
			if (f.getAnnotation(IgnoreField.class) != null) {
				TableColumn<T, Object> col = new TableColumn<T, Object>(formatToLabel(f.getName()));
	...
	public static String formatToLabel(String text) {
		...
		for (int i = 0; i < text.length(); i++) {
			...
			if (i == 0)
				result = String.valueOf(caractere).toUpperCase();
			else {
				if ((int) caractere >= 65 & (int) caractere <= 90)// if upper case
